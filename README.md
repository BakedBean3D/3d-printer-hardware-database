...
| `mount_hole_frame` | string | Optional, encouraged. Makes explicit what `terminal_faces` already assumes: the coordinate frame `bottom_mount_holes_xy`/`side_mount_holes_xy` are given in. `case_corner` = x=0,y=0 at a case corner (x along `length_mm`, y along `width_mm`); every case-body point lies in `[0, length_mm] x [0, width_mm]` (validated with a ±5mm tolerance for ears/slots that protrude past the nominal edge). `vendor_drawing` = some other datum (a drawing-sheet origin or non-case-relative convenience choice) — do not treat as case-relative. `null` = not yet established from the record's own provenance — never guessed from a sibling record's case family |
